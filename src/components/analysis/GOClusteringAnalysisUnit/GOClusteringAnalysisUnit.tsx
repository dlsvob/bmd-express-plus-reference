...
const GOClusteringAnalysisUnit: React.FC = () => {
  const logPrefix = '[GOClusteringAnalysisUnit v29 - Header Corrected]';
  ...
  const rowDataForClustering = useMemo(() => { // Clustering Data Prep
  ...
  const debouncedSubmitLogic = useMemo(() => debounce(() => { if (!selectedClusterForEnrichment || !enrichmentBackground || !categoryTableData) { setRunEnrichmentTrigger(false); setGeneListForEnrichment(null); return; } const genes = categoryTableData.filter(row => String(row.cluster) === String(selectedClusterForEnrichment)).map(row => (row.allGenes || '').split(';')).flat().map(g => g.trim()).filter(g => g.length > 0); const uniqueGenes = [...new Set(genes)]; if (uniqueGenes.length === 0) { message.warning(`No genes found in cluster ${selectedClusterForEnrichment} to submit.`); setRunEnrichmentTrigger(false); setGeneListForEnrichment(null); return; } console.log(`${logPrefix} Submitting ${uniqueGenes.length} unique genes for enrichment (Cluster ${selectedClusterForEnrichment}, BG: ${enrichmentBackground})`); setGeneListForEnrichment(uniqueGenes); setRunEnrichmentTrigger(true); }, 300), [selectedClusterForEnrichment, enrichmentBackground, categoryTableData, logPrefix]);
    ...
    <div className={styles.clusteringRoot}>
      ...
      <Spin spinning={isLoading && !!activeClusteringRef} tip={spinTip}>
        ...
        {!isLoadingRaw && activeClusteringRef && !error && (
          <>
            {!hasActiveDataToCluster && !isLoading && (<Empty description="No suitable category data..." />)}
            {hasActiveDataToCluster && !hasActiveResults && !isLoading && pyodideResult && (<Empty description="No categories found after processing..." />)}
            {hasActiveDataToCluster && !hasActiveResults && !isLoading && !pyodideResult && !pyodideError && (<Empty description="Clustering data processed, but no results returned." />)}
            ...
            {hasActiveResults && (
              <Space direction="vertical" size="large" style={{ width: '100%', padding: '0 16px' }}>
                ...
                <Card size="small" bordered={false} className={styles.innerSectionCard}>
                  <Row gutter={[16, 16]}>
                    <Col xs={24} md={4} lg={3}> <CustomLegends cardTitle="Ref Clusters" colorItems={legendColorItems} highlightedLabelsSet={highlightedRefClusterIdsSet} presentClusterIds={presentClusterIds} onToggleColorVisibility={handleToggleHighlightRefCluster} onToggleShapeVisibility={() => { }} onToggleSizeVisibility={() => { }} showColor={true} showShape={false} showSize={false} /> </Col>
                      ...
                      <Row gutter={[16, 16]}>
                        <Col xs={24} lg={14}>
                          <div style={{ minHeight: `500px`, border: '1px solid #f0f0f0', borderRadius: '4px', padding: '8px', position: 'relative' }}>
                            ...
                            ) : (<div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '400px' }}><Empty description={scatterPlotData === null ? "Preparing plot data..." : "No categories match filter."} /></div>)}
                          </div>
                        </Col>
                        <Col xs={24} lg={10}> <GOClusteringSummaryTable dataSource={summaryTableData} loading={isPyodideLoading} /> </Col>
                      </Row>
                    </Col>
                  </Row>
                </Card>
                ...
                {runEnrichmentTrigger && geneListForEnrichment && enrichmentBackground && selectedClusterForEnrichment && (
                  <Card size="small" bordered={false} className={styles.innerSectionCard} >
                    <GeneEnrichmentAnalysis geneList={geneListForEnrichment} backgroundType={enrichmentBackground} analysisName={`Cluster ${selectedClusterForEnrichment} (${activeAnalysisName})`} triggerRun={runEnrichmentTrigger} maxNodesToShow={networkNodesCount} />
                  </Card>
                )}
                ...
                <Card size="small" title={`Clustered Category Details (${filteredCategoryTableData.length} items)`} bordered={false} className={styles.innerSectionCard}>
                  <Row gutter={[16, 16]}> <Col span={24}> <GOClusteringDetailsTable dataSource={filteredCategoryTableData} loading={isPyodideLoading} /> </Col> </Row>
                </Card>

                {/* *** Enrichment Controls Card (Restored Position) *** */}
                <Card size="small" bordered={false} className={styles.innerSectionCard}>
                  <AnalysisControls
                    isExportDisabled={true} onCopy={() => { }} onExport={() => { }}
                    networkNodesCount={networkNodesCount} onNetworkNodesCountChange={handleNetworkNodesChange}
