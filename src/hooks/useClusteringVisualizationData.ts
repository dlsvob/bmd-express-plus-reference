...
// Import SummaryRow from clusteringUtils
import { SummaryRow } from '../utils/clusteringUtils';
// Import RankedCategoryRow from useProcessedClusteringData hook file
import type { RankedCategoryRow } from './useProcessedClusteringData'; // <<< CORRECTED IMPORT ADDED HERE
...
// Interface using the correctly imported RankedCategoryRow
...
export function useClusteringVisualizationData({
    ...
    // useMemo hook for clusterColorMap (No changes needed inside)
    ...
    // useMemo hook for jitterMap (No changes needed inside)
    ...
    // useMemo hook for scatterPlotData and presentClusterIds (No changes needed inside)
    ...
    }, [categoryTableData, summaryTableData, referenceDataMap, clusterColorMap, jitterMap]);

    // useMemo hook for legendColorItems (No changes needed inside)
    const legendColorItems = useMemo((): [string, string][] => {
        ...
        sortedClusterIds.forEach((clusterId) => {
            ...
            if (color) { items.push([String(clusterId), color]); }
        });
    ...
    // Return statement (No changes needed)
    return {
        clusterColorMap,
        jitterMap,
        scatterPlotData,
